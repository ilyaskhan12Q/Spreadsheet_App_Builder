"""
core/compiler/region_templates/pos.py — POS app layout template.

Produces a deterministic Blueprint for point-of-sale applications:
  Row 1-2:  Title header (merged)
  Row 3:    Spacer
  Row 4+:   Input form sections (label | value pairs)
  ...       Spacer
...
# Width of the main content area in columns
TOTAL_COLS = 5


def compile(spec: AppSpec, tokens: ResolvedTokens) -> Blueprint:
    """Compile a POS AppSpec into a complete Blueprint.
    ...
    cursor_row = 1  # Current row position (1-based)
    ...
    for r in range(header_start, header_end + 1):
        for c in range(1, TOTAL_COLS + 1):
            header_cell_ids.append(cell_ref(r, c))
    ...
    cursor_row = header_end + 2  # spacer row

    # ── Process sections in order ─────────────────────────────────
    for section in spec.sections:
        if section.section_type == SectionType.HEADER:
            # Already handled as title header; skip duplicate
            # but still register fields
            for fld in section.fields:
                registry.register(section.section_id, fld.name, cell_ref(1, 1))
            continue
        ...
        # Merge section title across 2 columns
        merges.append(MergeConfig(
            range=cell_range(cursor_row, 1, cursor_row, 2)
        ))
        ...
        if section.section_type in (
            ...
            # Input form: label | value pairs, one field per row
            for fld in section.fields:
                ...
                fmt = resolve_format_hint(
                    fld.format_hint or field_type_to_format_hint(fld.field_type)
                )
                ...
                elif fld.field_type == FieldType.BUTTON:
                    cells.append(Cell(
                        cell_id=value_ref,
                        value=fld.name,
                        style=make_style(
            ...
            # Data table: header row + repeatable data rows
            # Header row
            ...
            for ai, fld in enumerate(section.fields):
                ...
                if fld.field_type == FieldType.BUTTON:
                    # Check for style hints in actions list
            ...
            # KPI cards: label | value pairs across columns
            col = 1
            for fld in section.fields:
        ...
        cursor_row += 1  # spacer row

    # ── Build Meta ────────────────────────────────────────────────
    col_widths: dict[str, float] = {}
    for c in range(1, TOTAL_COLS + 1):
        letter = cell_ref(1, c)[0]  # Just the column letter
        if c == 1:
            col_widths[letter] = tokens.style.col_width_label
        else:
